...
function toggleLightAndDarkMode() {
	...
	if (globalState.lightMode) {
		color = "black";
		globalState.lightMode = false;
	} else {
		color = "white";
		globalState.lightMode = true;
	}
	
	//document.getElementsByClassName('app').style.backgroundColor = color;
	console.log(`${color}`);
}
*/
...
class HeaderComponent {
	...
	view() {
		return m("header", {class:"header", style:{"border-bottom-color":"var(--textColor)"}},
			[
				m("svg", {class:"header__icon header__icon--mode", id:"switchModeButton", "stroke":"currentColor", "fill":"currentColor", "stroke-width":"0", "viewBox":"0 0 24 24", "height":"1em", "width":"1em", "xmlns":"http://www.w3.org/2000/svg",
							onclick: () => {
									let color = ""

									//switching the Mode from light to dark and vice versa
									if (globalState.lightMode) {
										color = globalState.darkTheme.color;
										globalState.lightMode = false;
									} else {
										color = globalState.lightTheme.color;
										globalState.lightMode = true;
									}
									
									//configuring the style-properties in order to the actual theme
									let container = document.getElementById('container');
									container.style.backgroundColor = color;
									/*
									Placeholder for further toggling of HTML-elements:

									//toggling the svg
									let toggleSVG = document.getElementById("switchModeButton");
